...
public class ProtocolUtils {
    ...
    private static final Cache<ServerCommonPacketListenerImpl, IdentifierSelector> SELECTOR_CACHE =
        CacheBuilder.newBuilder().expireAfterAccess(30, TimeUnit.SECONDS).build();
    private static final Function<ByteBuf, RegistryFriendlyByteBuf> BUF_DECORATOR =
        buf -> buf instanceof RegistryFriendlyByteBuf registry
            ? registry
    ...
    public static void sendBytebufPacket(@NotNull ServerPlayer player, Identifier id,
                                         Consumer<? super RegistryFriendlyByteBuf> consumer) {
    ...
    public static void sendBytebufPacket(@NotNull Context context, Identifier id,
                                         Consumer<? super RegistryFriendlyByteBuf> consumer) {
        RegistryFriendlyByteBuf buf = decorate(Unpooled.buffer());
        consumer.accept(buf);
    ...
    public static IdentifierSelector createSelector(ServerCommonPacketListenerImpl common) {
        ...
        selector = new IdentifierSelector(new Context(common.getOwner(), common.connection), player);
        if (player != null) {
